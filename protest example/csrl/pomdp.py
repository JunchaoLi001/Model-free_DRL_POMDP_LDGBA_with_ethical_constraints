...
class GridPOMDP():
    ...
    def get_transition_prob(self,state,action_name):
        ...
        # South 'D'
        if action_name not in ['U', 'UL', 'UR'] and state[0]+1 < n_rows and self.structure[state[0]+1][state[1]] != 'B':
            states.append((state[0]+1,state[1]))
            probs.append(self.p if action_name=='D' else (1-self.p)/4)
        # North 'U'
        if action_name not in ['D', 'DL', 'DR'] and state[0]-1 >= 0 and self.structure[state[0]-1][state[1]] != 'B':
            states.append((state[0]-1,state[1]))
            probs.append(self.p if action_name=='U' else (1-self.p)/4) 
        # West 'L'
        if action_name not in ['R', 'UR', 'DR'] and state[1]-1 >= 0 and self.structure[state[0]][state[1]-1] != 'B':
            states.append((state[0],state[1]-1))
            probs.append(self.p if action_name=='L' else (1-self.p)/4)
        # East 'R'
        if action_name not in ['L', 'UL', 'DL'] and state[1]+1 < n_cols and self.structure[state[0]][state[1]+1] != 'B':
            states.append((state[0],state[1]+1))
            probs.append(self.p if action_name=='R' else (1-self.p)/4)
        
        # South_West 'DL'
        if action_name not in ['U', 'UR', 'R'] and state[0]+1 < n_rows and state[1]-1 >= 0 and self.structure[state[0]+1][state[1]-1] != 'B':
            states.append((state[0]+1,state[1]-1))
            probs.append(self.p if action_name=='DL' else (1-self.p)/4)
        # South_East 'DR'
        if action_name not in ['U', 'UL', 'L'] and state[0]+1 < n_rows and state[1]+1 < n_cols and self.structure[state[0]+1][state[1]+1] != 'B':
            states.append((state[0]+1,state[1]+1))
            probs.append(self.p if action_name=='DR' else (1-self.p)/4)
        # North_West 'UL'
        if action_name not in ['D', 'DR', 'R'] and state[0]-1 >= 0 and state[1]-1 >= 0 and self.structure[state[0]-1][state[1]-1] != 'B':
            states.append((state[0]-1,state[1]-1))
            probs.append(self.p if action_name=='UL' else (1-self.p)/4) 
        # North_East 'UR'
        if action_name not in ['D', 'DL', 'L'] and state[0]-1 >= 0 and state[1]+1 < n_cols and self.structure[state[0]-1][state[1]+1] != 'B':
            states.append((state[0]-1,state[1]+1))
            probs.append(self.p if action_name=='UR' else (1-self.p)/4)
